...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      } else { setStatus(`Error: ${data.error || 'Registration failed'}`); }
    } catch (error) { setStatus('Network error. Please try again.'); }
  };
    ...
    <section className="min-h-screen flex items-center justify-center pt-16 bg-gradient-to-br from-surface-50 via-white to-brand-50/30">
      <div className="w-full max-w-md px-6">
        <div className="text-center mb-8">
          ...
          <p className="text-surface-500 mt-1">Get started with PriceScope</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-surface-200 shadow-card p-8">
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            {status && (
              <div className={`text-sm text-center py-2 px-3 rounded-lg ${status.includes('Error') || status.includes('Network') ? 'bg-red-50 text-red-600' : status.includes('created') ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-600'}`}>
                {status}
              </div>
            )}
