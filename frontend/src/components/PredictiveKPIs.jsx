...
const PredictiveKPIs = ({ data }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full mt-10">
      {kpis.map((kpi, index) => (
        <div key={index} className={`relative bg-[#0a0a0f]/80 backdrop-blur-xl border-l-[3px] border-y-0 border-r-0 ${kpi.borderColor} px-6 py-6 min-h-[160px] flex flex-col justify-center rounded-r-2xl shadow-2xl hover:shadow-[0_0_40px_rgba(0,0,0,0.8)] transition-all duration-500 overflow-hidden group`}>
          ...
          <div className="flex justify-between items-start mb-3 relative z-10 w-full">
            ...
            <span className={`w-2 h-2 rounded-full animate-pulse flex-shrink-0 ml-2 ${kpi.borderColor.replace('border-', 'bg-')}`}></span>
          </div>
          
          <div className={`text-3xl lg:text-4xl font-mono font-black mb-1 tracking-tighter ${kpi.color} relative z-10 drop-shadow-[0_0_15px_currentColor] break-words`}>
            {kpi.value || "N/A"}
          </div>
          
          <p className="text-gray-500 font-mono text-[10px] uppercase tracking-widest relative z-10 mt-1">{kpi.subtitle}</p>
